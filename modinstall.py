...
parser.add_argument('--quick-exit', action='store_true') #Only used when exiting.
    ...
    if r.status_code == 200:
        ...
        if type == "file":
            ...
            if rzip.status_code == 200:
                ...
                silent_exit(exit_result.Download_Failed)
        else:
            return r.json()["version"]
    else:
        silent_exit(exit_result.Fetch_Latest_Failed)
...
def update_override(section, key, value):
    ...
    if before:
        ...
        silent_print(f'\033[42m{bcolors.BLACK}+ {bcolors.HEADER}[{section}]{bcolors.BLACK}: {key}{bcolors.BLACK} = {config[section][key]}{bcolors.ENDC}')
...
def main():
    ...
    if not os.path.exists(path): 
        ...
        silent_exit(exit_result.Bopimo_Not_Installed)
    ...
    if already_installed() and not run_args.reinstall:
        if not run_args.uninstall: #Update Mode
            ...
            silent_print(f'Instaled Version: {bcolors.OKCYAN}v{bopimod_version}{bcolors.ENDC}')
            ...
            if update_type:
                ...
                silent_exit(exit_result.Update_Sucess)

            else:
                silent_print('No updates found.')
                silent_exit(exit_result.Unchanged)
            ...
            silent_exit(exit_result.Uninstall_Sucess)
        ...
        silent_exit(exit_result.Install_Sucess)
